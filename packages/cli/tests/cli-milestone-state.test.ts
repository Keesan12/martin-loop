...
function makeState(overrides: Partial<MilestoneState> = {}): MilestoneState {
  return {
    ...
    feedback: { shownCount: 0, scores: [], featureVotes: [], email: null, optedOut: false },
...
describe("recordRunAndGetPrompt — suppressUntilRun", () => {
  ...
  it("allows GTM prompt once suppressUntilRun threshold is passed", async () => {
    ...
    mockStateFile(makeState({
      ...
      feedback: { shownCount: 1, lastShownAtRunCount: 5, lastShownAtSavedUsd: 1, scores: [], featureVotes: [], email: null, optedOut: false },
    }));
...
// ---------------------------------------------------------------------------
// recordRunAndGetPrompt — feedback recurrence
// ---------------------------------------------------------------------------

describe("recordRunAndGetPrompt — feedback recurrence", () => {
  it("fires the first feedback prompt at the fifth successful run", async () => {
    mockStateFile(makeState({
      successfulRunCount: 4,
      runCount: 4,
      waitlist: { status: "declined", declinedCount: 2, email: null, shownAt: null },
    ...
    const result = await runWithTty(BASE_INPUT);
    expect(result.interactivePrompt).toEqual({ kind: "feedback" });
  });

  it("does not recur on ordinary runs before fifteen more successful runs", async () => {
    mockStateFile(makeState({
      successfulRunCount: 18,
      runCount: 18,
      totalSavedUsd: 2,
      savingsConfidence: "confirmed",
      waitlist: { status: "declined", declinedCount: 2, email: null, shownAt: null },
      star: { shownCount: 0, confirmed: true },
      feedback: {
        shownCount: 1,
        lastShownAtRunCount: 5,
        lastShownAtSavedUsd: 1,
        scores: [],
        featureVotes: [],
        email: null,
        optedOut: false,
  ...
  it("recurs on ordinary runs at fifteen more successful runs", async () => {
    mockStateFile(makeState({
      successfulRunCount: 19,
      runCount: 19,
      totalSavedUsd: 2,
      savingsConfidence: "confirmed",
      waitlist: { status: "declined", declinedCount: 2, email: null, shownAt: null },
  ...
  it("does not recur for rollback or verifier events before ten more successful runs", async () => {
    mockStateFile(makeState({
      successfulRunCount: 13,
      runCount: 13,
      totalSavedUsd: 2,
      savingsConfidence: "confirmed",
      waitlist: { status: "declined", declinedCount: 2, email: null, shownAt: null },
    ...
    const result = await runWithTty({
      ...BASE_INPUT,
      estimatedUncontrolledUsd: 2.20,
      rollbackTaken: true,
    });
    expect(result.interactivePrompt).toBeNull();
  });

  it("recurs for rollback or verifier events at ten more successful runs", async () => {
    mockStateFile(makeState({
