...
function assertReleaseTreeClean() {
  try {
    const status = execFileSync(
      "git",
      ["status", "--porcelain", "--", "packages/mcp"],
      { cwd: repositoryRoot, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
    ).trim();

    if (status) {
      throw new Error(`Uncommitted changes exist under packages/mcp:\n${status}`);
    }
  } catch (error) {
...
async function verifyStaging() {
  ...
  for (const requiredPath of required) {
    const exists = requiredPath.endsWith("sdk")
      ? await isDirectory(requiredPath)
      : await isFile(requiredPath);
    if (!exists) throw new Error(`Required staged path is missing: ${requiredPath}`);
  }

  for (const forbidden of [".env", ".env.local"]) {
    if (await isFile(path.join(stagingRoot, forbidden))) {
      throw new Error(`Forbidden file included in staging: ${forbidden}`);
    }
  }
}
...
async function build() {
  ...
  await run(commandForPlatform("pnpm"), ["build"], { cwd: packageRoot });
  ...
  await run(commandForPlatform("npm"), [
    "install", "--omit=dev", "--ignore-scripts", "--no-audit", "--no-fund", "--package-lock=false"
  ], { cwd: serverRoot });

  await verifyStaging();
  await run(commandForPlatform("npx"), ["--yes", "@anthropic-ai/mcpb@2", "validate", stagingRoot]);
  await run(commandForPlatform("npx"), ["--yes", "@anthropic-ai/mcpb@2", "pack", stagingRoot, bundlePath]);
