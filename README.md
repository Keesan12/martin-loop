...
**Install** — run `npx -y martin-loop@0.5.4 start`, or install it globally with `npm install -g martin-loop@0.5.4`.
...
**MCP** — install `@martinloop/mcp@0.5.4` in a supported host or generate host configuration with `martin mcp print-config`.
...
Release notes for the current root package: [MartinLoop 0.5.4](./docs/release/OSS-0.5.4-RELEASE-NOTES.md).
...
npx -y martin-loop@0.5.4 demo
cd martin-loop-demo
npm install
npx -y martin-loop@0.5.4 run "Summarize the demo workspace and prove tests still pass" --verify "npm test" --budget-usd 2 --max-iterations 1 --json
npx -y martin-loop@0.5.4 dossier --latest --json
npx -y martin-loop@0.5.4 share --latest --json
```

For deterministic installs, pin the package line (`martin-loop@0.5.4`) or use `martin-loop@latest`. Plain `npx martin-loop` can resolve a stale local cache on some machines.
...
The root `martin-loop` package, standalone `@martinloop/mcp` package, plugin metadata, and MCPB product version are aligned at `0.5.4`. The MCPB manifest schema remains `0.3`.
...
- `0.5.4` governed-autonomous execution and proof-surface release
