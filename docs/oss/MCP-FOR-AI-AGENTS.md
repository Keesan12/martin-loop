...
`@martinloop/mcp@0.3.9` is the current standalone MCP source line for MartinLoop. The live npm baseline remains `0.3.6` until the `0.3.9` release is published, and follow-on standalone releases should still be cut only when the MCP surface itself changes.
...
- `0.3.9` is the current standalone MCP source line.
- `0.3.6` remains the live npm baseline until `0.3.9` ships.
