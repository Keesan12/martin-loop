...
| FL-053 | `public_repo_direct_write` | Agent writes code directly to the public release repo instead of routing through the internal OSS repo first | Block commit via PreToolUse hook matching path; require all changes to be staged through ML_Core_OSS_Internal before any public push; credential separation (session has no push access to public remote) is the load-bearing control | PreToolUse hook exit code 2 + git credential scope |
