...
test("MCPB product versions stay aligned without confusing schema version", async () => {
  const packageJson = JSON.parse(await read("packages/mcp/package.json"));
  const serverJson = JSON.parse(await read("packages/mcp/server.json"));
  const manifest = JSON.parse(await read("packages/mcp/mcpb/manifest.json"));

  assert.equal(serverJson.version, packageJson.version);
  assert.equal(manifest.version, packageJson.version);
  assert.equal(manifest.manifest_version, "0.3");
  assert.notEqual(manifest.manifest_version, manifest.version);
});
