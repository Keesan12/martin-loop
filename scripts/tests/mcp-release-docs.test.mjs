...
test("version ledger separates live public truth from the pending release target", async () => {
  ...
  assert.match(ledger, new RegExp(escapeRegex("live npm dist-tag `latest`: `0.6.5`")));
  assert.match(ledger, new RegExp(escapeRegex("live public GitHub release: `v0.6.5`")));
  assert.match(ledger, new RegExp(escapeRegex("live public GitHub release: `mcp-v0.6.5`")));
  assert.match(ledger, /root public baseline: `\d+\.\d+\.\d+`/);
  assert.match(ledger, /live public GitHub release: `v\d+\.\d+\.\d+`/);
  assert.match(
    ledger,
    new RegExp(escapeRegex("standalone MCP public baseline: `0.6.5`")),
  );
  assert.match(
    ledger,
